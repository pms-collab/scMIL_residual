...
This document is the method writeup for the `scMIL_residual` repository.

> **Data and pretrained embeddings are NOT included.**
> Users must obtain the dataset and generate embeddings (e.g., **scVI**) themselves.
> This repo provides the **modeling + training + evaluation + (planned) evidence extraction** pipeline.
...
This repo ships **no data/weights**. Reference scripts for preprocessing/embedding may exist under:
- `tools/preprocess/`
- `tools/embeddings/scvi/`
...
- Evidence extraction hooks (attribution-based; planned)
...
Reference: Ilse, Tomczak, Welling (2018), *Attention-based Deep Multiple Instance Learning*.
...
$$
z_{\text{prog}}=[\tilde z_s\mid a_{\text{keep}}]
$$
...
Reference: Cao, Mirjalili, Raschka (2020), *Rank consistent ordinal regression for neural networks with application to age estimation*, Pattern Recognition Letters.
...
- MIL: Ilse, Tomczak, Welling (2018).
- CORAL: Cao, Mirjalili, Raschka (2020).
