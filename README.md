...
- **Dual-branch attention MIL** on the same bag:
  - Disease branch: learns disease-common evidence (p(sick))
  - Severity branch: learns progression evidence (p(sev | sick))
- **Prototype disease subspace (K programs)** → well-defined projector onto disease row-space.
...
flowchart LR
  ...
  zd --> W[Prototypes W (QR-orthonormal)]
  W --> P[Projector P = W^T W]

  zs --> R[Residual r = (I-P) z_s]
  R --> Sev[Severity head] --> psev[p(sev | sick)]

  zd --> Sick[Disease head] --> psick[p(sick)]
  psick --> Staging[Compose C/M/S]
...
- Conditional severity (`p(sev|sick)`) can be more stable than direct 3-class accuracy.
- Treat this as a **mechanistic probe for evidence discovery** under small donor N + noisy/proxy severity labels.
...
- Integrated Gradients on the `sev|sick` logit w.r.t. cell embeddings
